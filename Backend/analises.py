...
def calcular_kpis_vazao(df):
    ...
    kpis = {}
    mapa_vazao = {
        'ultima_vazao_lida': ('vazao', 'm³/h'),
        'vazao_mensal': ('mes', 'm³'),
        'vazao_safra': ('total', 'm³')
    }
    for nome_kpi, (key_map, unidade) in mapa_vazao.items():
        nome_coluna = MAPEAMENTO_COLUNAS.get(key_map)
        valor_final = 0
        if nome_coluna and nome_coluna in df.columns:
            serie_com_valores = df[nome_coluna][df[nome_coluna] > 0]
            if not serie_com_valores.empty:
                ultimo_indice_valido = serie_com_valores.last_valid_index()
                if ultimo_indice_valido is not None:
                    valor_final = df.loc[ultimo_indice_valido, nome_coluna]
        kpis[nome_kpi] = {"valor": valor_final, "unidade": unidade}
    return kpis